...
| Тест | Проверяет |
|---|---|
| `hal_gpio_test` | контракт `abl_gpio` на native-порте: конфигурация, вывод, toggle, симулированный вход, ошибки (NULL, неверный mode, неинициализированный пин), «прерывания не поддерживаются», а также **toggle по тени состояния** (регрессия на ESP32-C3: чтение площадки вместо выданного уровня) |
...
## Две роли: выданный уровень и площадка

Native-порт различает **выданный уровень** (`out_level`, тень для `toggle`) и
**уровень на площадке** (`pad_level`, то, что читает `abl_gpio_read`). Это не
педантизм: на реальном железе они расходятся — именно из-за этого ESP32-порт
«мигал еле заметно», пока `toggle()` читал площадку вместо тени. Симулировать
расхождение (нагрузка «подсадила» выход) можно так:

```c
abl_gpio_write(&led, true);            /* выдали высокий уровень */
abl_native_gpio_inject(&led, false);   /* площадку подтянуло вниз нагрузкой */
abl_gpio_toggle(&led);                 /* всё равно перейдёт в низкий */
```
